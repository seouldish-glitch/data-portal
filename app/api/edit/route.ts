...
export async function POST(req: NextRequest) {
  try {
    ...
    const achievementPhotos = [
      ...(formData.getAll("existing_achievementPhotos") as string[]),
      ...(await uploadFiles("achievementPhotos")),
    ];
    ...
    switch (category) {
      case "sports": {
        ...
        if (!groupPhotoUrl)
          groupPhotoUrl = formData.get("existing_groupPhoto") as string | null;
        const logoAndPhotos = [
          ...(formData.getAll("existing_mediaUploads") as string[]),
          ...(await uploadFiles("mediaUploads")),
        ];
      ...
      case "prefects": {
        ...
        if (!guildLogo)
          guildLogo = formData.get("existing_guildLogo") as string | null;
        const officialPhotos = [
          ...(formData.getAll("existing_officialPhotos") as string[]),
          ...(await uploadFiles("officialPhotos")),
        ];
      ...
      case "houses": {
        ...
        if (!houseFlagUrl)
          houseFlagUrl = formData.get("existing_houseFlag") as string | null;
        const actionPhotos = [
          ...(formData.getAll("existing_actionPhotos") as string[]),
          ...(await uploadFiles("actionPhotos")),
        ];
      ...
      case "obu": {
        ...
        if (!logoUrl)
          logoUrl = formData.get("existing_obuLogo") as string | null;
