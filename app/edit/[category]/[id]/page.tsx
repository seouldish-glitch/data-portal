...
export default function EditPage({ params }: { params: { category: string, id: string } }) {
  ...
  if (!config) {
    return (
      <div className="page-wrapper" style={{ padding: '5rem', textAlign: 'center' }}>
        <h2>Form Not Found</h2>
        <Link href="/" style={{ color: 'var(--sbc-green)' }}>Return Home</Link>
      </div>
    );
  }

  if (isAuthenticated === null || isLoadingData) {
    return <div className="page-wrapper" style={{ padding: '5rem', textAlign: 'center' }}><h2>Loading...</h2></div>;
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`/api/edit?category=${params.category}&id=${params.id}`, {
        method: 'POST',
        body: formData,
      });
  ...
  return (
    <div className="page-wrapper">
      ...
      {submitStatus === 'success' && (
        <div className="feedback-overlay">
          <div className="feedback-card">
            ...
            <h2 style={{ color: 'var(--sbc-green)', marginBottom: '1rem' }}>Update Successful</h2>
            <p style={{ color: 'var(--text-secondary)', marginBottom: '2rem', lineHeight: '1.6' }}>
              Your records have been successfully updated and archived to MongoDB. Thank you for contributing to the 161st Anniversary Digital Project.
            </p>
            <button onClick={() => setSubmitStatus('idle')}
              style={{ background: 'var(--sbc-green)', color: '#050505', border: 'none', borderRadius: '8px', padding: '0.8rem 2rem', fontWeight: '800', cursor: 'pointer', textTransform: 'uppercase', letterSpacing: '1px' }}>
              Done
            </button>
          </div>
        </div>
      ...
      {submitStatus === 'error' && (
        <div className="feedback-overlay">
          <div className="feedback-card" style={{ borderColor: '#ef4444' }}>
            <div className="feedback-success-icon" style={{ background: 'rgba(239, 68, 68, 0.1)', color: '#ef4444' }}>✕</div>
            <h2 style={{ color: '#ef4444', marginBottom: '1rem' }}>Submission Failed</h2>
            <p style={{ color: 'var(--text-secondary)', marginBottom: '2rem', lineHeight: '1.6' }}>{errorMessage}</p>
            <button onClick={() => setSubmitStatus('idle')}
              style={{ background: '#ef4444', color: '#ffffff', border: 'none', borderRadius: '8px', padding: '0.8rem 2rem', fontWeight: '800', cursor: 'pointer', textTransform: 'uppercase', letterSpacing: '1px' }}>
              Try Again
            </button>
          </div>
        </div>
      ...
      <main className="main-content" style={{ maxWidth: '800px', margin: '0 auto', paddingTop: '4rem' }}>
        <Link href="/" style={{ color: 'var(--sbc-green)', textDecoration: 'none', marginBottom: '2.5rem', display: 'inline-flex', alignItems: 'center', gap: '0.5rem', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '1px', fontSize: '0.9rem' }}>
          <span>&larr;</span> Back to Dashboard
        </Link>
        <header style={{ marginBottom: '3.5rem', borderBottom: '1px solid rgba(255, 255, 255, 0.08)', paddingBottom: '2.5rem' }}>
          <h1 style={{ fontSize: 'clamp(2rem, 4vw, 2.75rem)', marginBottom: '1rem', color: 'var(--sbc-green)', fontFamily: "'Cinzel', serif", letterSpacing: '1px' }}>{config.title}</h1>
          <p style={{ color: 'var(--text-secondary)', fontSize: '1.05rem', lineHeight: '1.6', fontWeight: 500 }}>{config.subtitle}</p>
        </header>
        <div style={{ background: 'rgba(34, 197, 94, 0.1)', borderLeft: '4px solid var(--sbc-green)', padding: '1rem', marginBottom: '2.5rem', color: 'var(--text-primary)' }}>
          <strong style={{ color: 'var(--sbc-green)' }}>Note:</strong> Please ensure you upload pictures of achievements, including historical/old photos if available.
        </div>
        <form ref={formRef} onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '2.5rem' }}>
          {config.fields.map((field, i) => (
            <div key={i} className="form-group" style={{ animationDelay: `${0.1 * i}s` }}>
              ...
              {field.type === 'file' && (
                <div className="form-file-wrapper">
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                        {getExistingFiles(field.name).map((url: string, idx: number) => (
                          <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(0,0,0,0.3)', padding: '0.5rem 1rem', borderRadius: '6px' }}>
                            <a href={url} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--sbc-green)', fontSize: '0.85rem', textDecoration: 'none', maxWidth: '70%', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{url.split('/').pop()}</a>
                  ...
                  <input
                    ...
                    name={field.name}
                    accept={field.accept}
                    required={field.required && getExistingFiles(field.name).length === 0 && !selectedFiles[field.name]}
